...
class TestPublicIndex(unittest.TestCase):
    class EmbeddedPuzzle(TypedDict):
        sourceSeed: str
        rows: list[str]
        acrossClues: list[str]
        downClues: list[str]

    @staticmethod
    def _decode_js_string_list(block: str) -> list[str]:
            ...
            puzzles.append(
                {
                    ...
                    "rows": self._decode_js_string_list(match.group(2)),
                    "acrossClues": self._decode_js_string_list(match.group(3)),
                    "downClues": self._decode_js_string_list(match.group(4)),
                }
            )
        return puzzles
    ...
    def test_embedded_puzzle_bank_never_repeats_clue_text(self) -> None:
        clues = []
        for puzzle in self._load_embedded_puzzles():
    ...
    def test_keyboard_and_modal_buttons_have_extra_vertical_space(self) -> None:
        ...
        self.assertIn("--key-font: 16px;", html)
        self.assertIn("--key-font: 14px;", html)
        self.assertRegex(html, re.compile(r"--key-pad-y:\s*14px;"))
        self.assertIn("--delete-key-pad-y: 11px;", html)
        self.assertIn("--key-side-pad: 4px;", html)
        self.assertIn("--key-height-adjust: 3px;", html)
        self.assertIn("background: var(--white);", html)
        self.assertIn("min-height: calc(var(--key-font) + (var(--key-pad-y) * 2) + 4px + var(--key-height-adjust));", html)
        self.assertIn("#delete-key {", html)
        self.assertIn("min-height: calc(var(--key-font) + (var(--delete-key-pad-y) * 2) + 4px);", html)
        self.assertIn("padding-top: var(--delete-key-pad-y);", html)
        self.assertIn("padding-bottom: var(--delete-key-pad-y);", html)
    ...
    def test_mobile_board_size_is_limited_by_available_viewport_height(self) -> None:
        ...
        self.assertIn("--mobile-topbar-height: 40px;", html)
        self.assertIn("--mobile-keyboard-buffer: 8px;", html)
        self.assertIn("--mobile-keyboard-row-height: calc(var(--key-font) + (var(--key-pad-y) * 2) + 4px + var(--key-height-adjust));", html)
        self.assertIn("--mobile-keyboard-action-height: calc(var(--key-font) + (var(--delete-key-pad-y) * 2) + 4px);", html)
        self.assertIn("--mobile-keyboard-height: calc((var(--mobile-keyboard-row-height) * 3) + var(--mobile-keyboard-action-height) + (var(--key-gap) * 3) + var(--mobile-keyboard-buffer));", html)
        ...
        self.assertIn("padding: 6px 12px;", html)
    ...
    def test_clue_card_uses_fixed_height_and_fit_text_logic(self) -> None:
        ...
        self.assertIn("function cellNeedsEntry(row, col) {", html)
        self.assertIn("function getNextCellNeedingEntry(word, startIndex) {", html)
        ...
        self.assertIn('name: "preferred clue cell picks first unresolved cell"', html)
        self.assertIn('name: "down preferred cell skips solved crossings"', html)
        ...
        self.assertIn('name: "typing skips solved crossing letters before advancing"', html)
        self.assertIn('name: "typing last cell advances to the next clue"', html)
        ...
        self.assertIn('name: "score pill increments when a word is newly solved"', html)
        self.assertIn('name: "solved clue card stays highlighted while active word is solved"', html)
        self.assertIn('name: "solved words keep board cells neutral until cleared"', html)

    def test_score_pill_tracks_solved_words_without_tinting_board_blue(self) -> None:
        html = INDEX_HTML.read_text(encoding="utf-8")
        ...
        self.assertIn(".score-pill.celebrate {", html)
        self.assertIn("@keyframes score-pill-pop {", html)
        self.assertIn("@keyframes score-pill-spark {", html)
        self.assertIn("@keyframes score-pill-ring {", html)
        self.assertIn("@keyframes score-burst-float {", html)
        self.assertIn("function updateScorePill(gainedCount) {", html)
        self.assertIn("function syncSolvedProgress(shouldCelebrate) {", html)
        self.assertIn("scoreCountEl.textContent = String(solvedWordKeys.size);", html)
        self.assertIn('scoreBurstEl.textContent = "+" + String(gainedCount);', html)
        self.assertIn("syncSolvedProgress(true);", html)
        self.assertIn("syncSolvedProgress(false);", html)
        self.assertNotIn(".cell.solved {", html)
        self.assertNotIn("function isCellInSolvedWord(row, col) {", html)
        self.assertNotIn("const solvedNow = isCellInSolvedWord(r, c);", html)
        self.assertNotIn('cell.classList.add("solved")', html)
