...
from byewords.grid import distinct_entries, grid_columns, make_grid
...
def generate_puzzle(
    ...
    if set(DEFAULT_DEMO_ENTRIES).issubset(lexicon_set):
        if not available_seeds:
            return build_demo_puzzle(clue_bank)
        if any(seed in DEFAULT_DEMO_ENTRIES for seed in available_seeds):
    ...
    for candidate_window in _candidate_windows(candidate_words):
        ...
        if attempt and not grids:
            grids = attempt
        if attempt and (not seed_word_set or _has_seeded_grid(attempt, seed_word_set)):
            grids = attempt
            break
    ...
    if not ranked:
        raise ValueError("unable to generate a valid 5x5 puzzle from the bundled lexicon")
